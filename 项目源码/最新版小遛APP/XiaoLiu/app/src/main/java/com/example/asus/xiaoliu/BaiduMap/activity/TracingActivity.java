package com.example.asus.xiaoliu.BaiduMap.activity;
...
import com.example.asus.xiaoliu.Async.SignAsy.FindLGSByOrderId;
import com.example.asus.xiaoliu.BaiduMap.model.CurrentLocation;
import com.example.asus.xiaoliu.BaiduMap.utils.BitmapUtil;
import com.example.asus.xiaoliu.BaiduMap.utils.CommonUtil;
import com.example.asus.xiaoliu.BaiduMap.utils.Constants;
import com.example.asus.xiaoliu.BaiduMap.utils.MapUtil;
import com.example.asus.xiaoliu.BaiduMap.utils.ToastUtils;
import com.example.asus.xiaoliu.BaiduMap.utils.TrackReceiver;
import com.example.asus.xiaoliu.BaiduMap.utils.ViewUtil;
import com.example.asus.xiaoliu.HomeActivity;
import com.example.asus.xiaoliu.MyApplication;
...
//        LiuGouShi = BmobUser.getCurrentUser(User.class).getUsername();
    ...
    private void init() {
        ...
        powerManager = (PowerManager) trackApp.getSystemService(getApplicationContext().POWER_SERVICE);
...
//        //
        ...
        if(intent.getStringExtra("identity")!=null&&intent.getStringExtra("identity").equals("狗狗主人")){
            LinearLayout linearLayout=findViewById(R.id.layout_tracing_bottom);
            linearLayout.setVisibility(View.GONE);
            LinearLayout linearLayout1=findViewById(R.id.layout_tracing_bottom_zr);
            linearLayout1.setVisibility(View.VISIBLE);

            oid=intent.getStringExtra("orderId");
            String entityName=intent.getStringExtra("entityName");
            ImageView imageView=findViewById(R.id.map_iv);
            TextView textView=findViewById(R.id.tv_name);
            new FindLGSByOrderId(imageView,getApplicationContext(),oid,textView).executeOnExecutor(Executors.newCachedThreadPool());
            HistoryTrackRequest historyTrackRequest=new HistoryTrackRequest(trackApp.getTag(),trackApp.serviceId,entityName);
...
//            });
            ...
            order.setEntityName(trackApp.entityName);
            order.update(oid,new UpdateListener() {
                @Override
                public void done(BmobException e) {
                    Log.e("entityname","更新成功");
                }
            });
...
//                                new JPushAsyncFinsh(getApplicationContext(),orderClientId,LiuGouShi).executeOnExecutor(Executors.newCachedThreadPool());
                                ...
                                intent.putExtra("signMark",String.valueOf(4));
                                intent.putExtra("oid",oid);
                                intent.setClass(TracingActivity.this, HomeActivity.class);
...
//                查询历史轨迹
                HistoryTrackRequest historyTrackRequest=new HistoryTrackRequest(trackApp.getTag(),trackApp.serviceId,trackApp.entityName);
//                long startTime = System.currentTimeMillis() / 1000 - 24 * 60 * 60;
                long startTime = System.currentTimeMillis() / 1000-24*60*60;
                long endTime = System.currentTimeMillis() / 1000;
                historyTrackRequest.setStartTime(startTime);
...
//                //查询实时位置 未写完
//                int returnType=0;
//                int activityTime=(int)(System.currentTimeMillis() / 1000 - 12 * 60 * 60);
//                int pageSize=1000;
//                int pageIndex=1;
//                trackApp.mClient.queryEntityList(trackApp.serviceId,"861916031983294",null,returnType,activityTime,pageSize,pageIndex,entityListener);
